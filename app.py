...
# =============================================================================
# REGRA DE OURO: st.set_page_config TEM de ser o primeiro comando do Streamlit!
st.set_page_config(
...
def carregar_dados_sheets(url: str) -> pd.DataFrame:
    try:
        ...
        # ⚠️ ARMADILHA ANTI-ESPAÇOS: Limpa espaços em branco invisíveis dos cabeçalhos
...
if coluna_vencimento in df_carteira_crua.columns:
    ...
    if datas_convertidas.isna().any():
        if 'Sem Data' not in opcoes_mes: opcoes_mes.append('Sem Data')
        if 'Sem Data' not in opcoes_dia: opcoes_dia.append('Sem Data')
else:
...
with col_pizza1:
    ...
    with st.container(height=480, border=True): 
        if coluna_grupo in df_graficos_filtrados.columns:
            ...
            fig2 = px.pie(
                df_g2, values=coluna_valor, names='Legenda', hole=0.4, 
